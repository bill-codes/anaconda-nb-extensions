...
define (['nbextensions/utils/main', 'components/codemirror/mode/diff/diff'], function(utils){
...
//      if (msg === '' && status === "merge") {
//          alert("You have nothing to commit yet.");
//      }
    }
...
//      }
  ...
  function getStatus(sha1, sha2) {
    ...
    function callback_script(msg) {
      var retval = msg.content.data.trim();
      console.log(retval);
    ...
    var rcm_dir = "get_ipython().ipython_dir"
...
//    var diff = "d";
  ...
  function writer(log) {
    ...
    ).append('Checkouting any revision point will <b>destroy</b> your current work.<br/>' +
      ...
      if (empty === false) {
        ...
        if (logsha !== undefined) {
          ...
          deleteBranch(oldsha);
            ...
            alert("You have to select just ONE to checkout or TWO to diff.");
        }
      } else {
        console.log(empty);
      }
